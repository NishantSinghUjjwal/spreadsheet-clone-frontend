...
const AddCellsLayout = ({ children, }: { children: React.ReactNode, }) => {
    ...
    return (
        <div className="flex h-[calc(100%_-_7.50rem)] w-full relative">
            <div className="flex flex-col w-[calc(100%_-_1.5rem)] h-full">
                {children}
                <button
                    className=" w-full h-6 min-h-6 text-xs bg-gray-200 hover:bg-gray-300 border-l border-gray-400 mt-auto flex items-center justify-center"
                    onClick={addRow}>
                    <Plus size={16} />
                </button>
            </div>
